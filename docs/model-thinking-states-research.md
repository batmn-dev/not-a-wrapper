...
**Tier mapping for the Open Questions.** **Product #1–8 are all Tier B** (preference/A-B), with two Tier-A floors that bound the choice: #6 (`Thought` is disfavored on faithfulness grounds, not free tone) and #4 (whose *content-sanitization* + *terminal-status* dimensions are Tier-A security gates tracked as Technical #9/#12 — the Product-#4 question itself is only the residual display-policy choice). The hard Tier-A constraints themselves are **findings, not open questions**.
...
- **`sendReasoning` default is `true`** (verified in installed `ai@6.0.78`, `dist/index.mjs:7275`; the `delta: part.text` wire mapping is in the reasoning-delta enqueue block `dist/index.mjs:7334-7339`, with `delta: part.text` on `:7337`). The v7 module name `to-ui-message-chunk.ts` does **not** exist in v6 — the mapping is inline. Reasoning is forwarded to the client by default — a *transport* default, not a *display* policy. (Sibling defaults: `sendSources=false`, `sendStart=true`, `sendFinish=true`.)
...
- **Finding: static disclosure is a prediction; durably-observed exposure should override it** *when captured*. Grok-by-model and OpenRouter-by-upstream make any static-only disclosure wrong. (On abort/fail nothing is captured — see the disclosure-source precedence under "Conflict resolution".)
