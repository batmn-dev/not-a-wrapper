 ...
 * before. With it on the sidebar reads a paginated recency window
 * (`chats.listForCurrentUserPaginated`) plus a small live pinned read
 * (`chats.getPinnedForCurrentUser`), so a chat write no longer re-reads the whole
 * collection. Temporary rollout lever — remove after the soak.
 *
