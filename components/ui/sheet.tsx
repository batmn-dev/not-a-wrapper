...
function SheetOverlay({ className, ...props }: SheetPrimitive.Backdrop.Props) {
  return (
    <SheetPrimitive.Backdrop
      ...
      className={cn(
        "bg-scrim-modal fixed inset-0 z-50 transition-opacity duration-150 data-ending-style:opacity-0 data-starting-style:opacity-0 supports-backdrop-filter:backdrop-blur-xs",
        className
      )}
      {...props}
    />
...
function SheetContent({
  ...
  /**
   * Optional class merged onto the backdrop. Omitted (the default) is
   * byte-identical to today — `cn(base, undefined)` === `base` — so the two
   * existing Sheet consumers are unaffected by construction.
   */
