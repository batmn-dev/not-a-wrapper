...
_Status_: designed 2026-07-05 (ADR-0009); not yet implemented — `durable-runtime.ts` still holds the helpers it will absorb.
...
The single canonical answer to "what is this turn doing right now?" — a discriminated union (`submitted → thinking(visible|opaque) → generating-image → tooling(toolNames) → awaiting-approval → generating → responding → settled`) derived by one pure first-match ladder in `lib/chat-messages/assistant-turn.ts` from the **Assistant turn view** plus one liveness fact (`isLast` + the row render status). Every loading/progress affordance on an assistant row — the activity trigger's live states, the generating shimmer, tool-progress labels, tool status chips, the panel's elapsed timer — is a PRESENTATION of this phase; none re-derives its own gate from raw parts/status. A sibling `deriveAssistantTurnIndicator` maps the phase to exactly one indicator value for the row's single indicator slot, so two simultaneous indicators are unrepresentable. Liveness rules: only the last turn, while THIS client's stream is submitted/streaming, is ever in a live phase; durable statuses override only when they assert a settled/paused outcome (aborted/failed/awaiting_approval) — durable `streaming`/`submitted` are never trusted for liveness (a stopped/dropped stream's server run lags its terminal transition, and raw part states freeze in place on abort, so part-state-driven indicators must read settled the moment the client's stream ends).
...
_Status_: implemented 2026-07-03 (branch `darknight/amusement-mile`).
