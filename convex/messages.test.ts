...
describe("message branch selection", () => {
  ...
  it("assigns unique missing branch indexes when clearing sparse siblings", async () => {
    const messages = [
      ...
      createMessage({
        id: "message_assistant_missing_1",
        orderId: 2,
        role: "assistant",
        content: "missing answer one",
        parentMessageId: asMessageId("message_user_1"),
      }),
      createMessage({
        id: "message_assistant_missing_2",
        orderId: 3,
        role: "assistant",
        content: "missing answer two",
        parentMessageId: asMessageId("message_user_1"),
      }),
    ]
    const { ctx, tables } = createMutationCtx({ messages })

    await clearSiblingSelectionForMutation(
      ctx,
      tables.messages,
      asMessageId("message_user_1"),
      "assistant",
      100
    )

    expect(
      tables.messages.map((message) => ({
        id: message._id,
        branchIndex: message.branchIndex,
        selected: message.selected,
      }))
    ).toMatchObject([
      { id: "message_user_1", branchIndex: 0, selected: true },
      { id: "message_assistant_existing", branchIndex: 2, selected: false },
      { id: "message_assistant_missing_1", branchIndex: 0, selected: false },
      { id: "message_assistant_missing_2", branchIndex: 1, selected: false },
    ])
  })

  it("assigns unique missing branch indexes when selecting sparse siblings", async () => {
    const messages = [
      createMessage({
        id: "message_user_1",
        orderId: 0,
    ...
    expect(
      ...
      { id: "message_assistant_existing", branchIndex: 2, selected: false },
      { id: "message_assistant_missing_1", branchIndex: 0, selected: false },
      { id: "message_assistant_missing_2", branchIndex: 1, selected: true },
    ])
  })
})
