...
function sha256Bytes(bytes: Uint8Array): Uint8Array {
  ...
  // Message lengths here are far below 2^32 bits; the high word stays 0.
...
/**
 * Constant-time hex-digest comparison. Both inputs are SHA-256 hex digests, so
 * an attacker never controls a value compared against the stored secret
 * preimage; this guards against timing on the digest itself.
 */
