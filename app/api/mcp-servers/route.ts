 ...
 *
 * Handles create/update of MCP server configurations. Encrypts auth values
 * server-side before storing in Convex. Auth + CSRF are enforced by the
...
export const PATCH = authenticatedRoute(
  async (request, { session, convex }) => {
    try {
      ...
      // Encrypt new auth value if provided, bound to the owner (AAD).
      if (authValue && (authType === "bearer" || authType === "header")) {
        const encrypted = encryptSecret(authValue, {
          kind: "mcpAuth",
          ownerId: session.userId,
